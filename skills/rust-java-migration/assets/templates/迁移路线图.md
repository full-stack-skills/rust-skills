...
18. 所有 `.rs` 文件最多 200 个物理行；超长对象按职责拆分但不混入第二个 Java 对象，测试代码与生产 `src/` 物理分离。
18. 仓库/产品级迁移必须建立非发布的 `<project>-test` workspace package，集中拥有整体功能验收；多 crate、binding/adapter 或源 system suite 尤其不得省略。
...
- `.rs` 文件 200 行上限、超长对象职责拆分和生产/测试代码物理隔离检查。
...
- [ ] 每个 `.rs` 文件不超过 200 个物理行；超长对象拆分文件只服务同一对象，生产 `src/` 不包含测试代码。
